...
const AdvertisementModel = ({ setShowAdvertisementModel }: AdvertisementModelProps) => {
      ...
      try {
        await httpClient("multipart/form-data")
          ...
          .then((res) => {
            ...
            if (res.status == 200) {
              toast.success("Advertisement created successfully. pls wait until admin approves");
            }
          }
  ...
  const fetchJobCategory = async () => {
    try {
      await httpClient()
        .get("/category/job/customer")
        .then((res) => setJobCAtegory(res?.data?.data));
    } catch (error) {
      console.log(error);
    }
  }
  ...
  return (
    <>
      ...
      <div className="fixed top-0 left-0 w-full h-screen flex items-center justify-center text-black z-50">
        <div className="relative flex flex-col md:flex-row gap-6 p-5 text-center font-medium  bg-white rounded-lg lg:w-5/12">
            ...
            >
              {adImg ?
                ...
                (<div {...getAdRootProps()}>
                  ...
                  />
                  <div
                    className={`w-full h-40 border-2 rounded border-dashed p-4 flex flex-col gap-2 items-center justify-center cursor-pointer border-black ${isDragActive ? "border-blue-500" : "border-black"
                      }`}
                  >
              ...
              {watchType == "JOB" ? <div className="flex items-center gap-4">
                <p>Category</p>
                <div>
                  <select
                    id="type"
